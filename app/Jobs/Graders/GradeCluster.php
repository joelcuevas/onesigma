...
class GradeCluster implements WorkflowableJob
{
    ...
    protected function gradeMetrics()
    {
        ...
        foreach ($this->team->children as $team) {
            $metrics = $team->getWatchedMetrics();
    ...
    protected function gradeSkills()
    {
        ...
        $scores = [];
        $teamsCount = $this->team->children->count();

        foreach ($this->team->children as $team) {
            foreach ($team->skillset->getSkills(false) as $i => $s) {
                $scores[$i] = ($scores[$i] ?? 0) + $s;
            }
        }

        foreach ($scores as $i => $s) {
            $scores[$i] = bcdiv($s / $teamsCount, 1, 2);
        }
